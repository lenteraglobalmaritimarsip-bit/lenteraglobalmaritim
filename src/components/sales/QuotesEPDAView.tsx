...
export const QuotesEPDAView: React.FC<QuotesEPDAViewProps> = ({ job, vessels, users, onDataSaved }) => {
  ...
  const openPreview = (print = false) => {
    ...
    const html = formatExportTable(enhanceExportHeader(buildDocument())).replace(/<div class="footer">[\s\S]*?<\/div>/, '').replaceAll('class="meta-col right"', 'class="meta-col right" style="padding-right:18px"').replaceAll('>No.</span>', '>No EPDA</span>');
